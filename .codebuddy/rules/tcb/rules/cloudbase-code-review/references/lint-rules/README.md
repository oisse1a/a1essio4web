...
function checkPlaceholderAccessKey(files) {
  const pattern =
    /accessKey\s*:\s*['"]<.+>['"]|accessKey\s*:\s*['"]envId['"]|accessKey\s*:\s*['"]YOUR_/;
...
function checkOldWebAuthApi(files) {
  ...
  for (const file of files.filter((f) =>
    /auth|login|register|guard|ProtectedRoute|App\.(tsx|jsx|ts|js)$/i.test(f),
  )) {
    const content = readFileSafe(file);
    if (!content) continue;
    content.split("\n").forEach((line, i) => {
...
function checkWxCloudDatabase(files) {
  for (const file of files.filter((f) =>
    /app\.(ts|tsx|js|jsx)$|backend\.(ts|tsx|js)$|main\.(ts|tsx|js)$/i.test(f),
  )) {
    const content = readFileSafe(file);
    if (!content) continue;
    content.split("\n").forEach((line, i) => {
      if (
        /wx\.cloud\.database\s*\(/.test(line) &&
        !file.includes("miniprogram") &&
        !file.includes("wx")
      ) {
        record(
          "NOSQL-001",
...
function checkOpenidManual(files) {
  for (const file of files) {
    ...
    content.split("\n").forEach((line, i) => {
      if (
        /['"]_openid['"]\s*[:=]\s*['"]/.test(line) &&
        /\.(add|insert|update|set)\s*\(/.test(content)
      ) {
        record(
          "NOSQL-004",
          "error",
...
function checkLocalhostDomain(files) {
  ...
  if (existsSync(pkgPath)) {
    ...
    if (
      pkg &&
      !all.includes("localhost:5173") &&
      !all.includes("CreateAuthDomain") &&
      !all.includes("authDomain")
    ) {
      record(
        "STORAGE001",
        "error",
...
function checkSensitiveRuntimeEcho(files) {
  ...
  const backendContent =
    /(createServer\s*\(|express\s*\(|exports\.main\s*=|kennethreitz\/httpbin|image:\s*.*httpbin)/i;
  for (const file of files) {
    const content = readFileSafe(file);
    if (!content) continue;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        /(headers\s*:\s*req\.headers|headers\s*:\s*event\.headers|env\s*:\s*process\.env)/.test(
          line,
        )
      ) {
        record(
